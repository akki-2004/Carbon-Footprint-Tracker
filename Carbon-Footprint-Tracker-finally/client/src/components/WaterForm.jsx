  ...
  return (
    <div className="form-container">
      ...
      <div className="form-group mt-8">
        <label className="block font-bold text-2xl mb-4 text-center text-white">What was your primary water source today?</label>
        {errors["What was your primary water source today?"] && (
          <p className="text-red-500 text-lg">{errors["What was your primary water source today?"]}</p>
        )}
        <select
      ...
      {/* Bathing Method */}
      <div className="form-group mt-8">
        <label className="block font-bold text-2xl mb-4 text-center text-white">How did you take a bath today?</label>
        {errors["How did you take a bath today?"] && (
          <p className="text-red-500 text-lg">{errors["How did you take a bath today?"]}</p>
        )}
        <select
          className="input-box text-lg p-4 border-2 border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      {usesTanker && (
        <div className="form-group mt-8">
          <label className="block font-bold text-2xl mb-4 text-center text-white">How many liters of bottled water did you consume today?</label>
          {errors["How many liters of bottled water did you consume today?"] && (
            <p className="text-red-500 text-lg">{errors["How many liters of bottled water did you consume today?"]}</p>
          )}
          <input
            type="number"
            className="input-box text-lg p-4 border-2 border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      {/* RO Purifier Usage */}
      <div className="form-group mt-8">
        <label className="block font-bold text-2xl mb-4 text-center text-white">Did you use an RO purifier today?</label>
        {errors["Did you use an RO purifier today?"] && (
          <p className="text-red-500 text-lg">{errors["Did you use an RO purifier today?"]}</p>
        )}
        <div className="radio-group space-x-8">
          {["Yes", "No"].map((option) => (
      ...
      {usedRO && (
        <div className="form-group mt-8">
          <label className="block font-bold text-2xl mb-4 text-center text-white">If yes, did you reuse the rejected water?</label>
          {errors["If yes, did you reuse the rejected water?"] && (
            <p className="text-red-500 text-lg">{errors["If yes, did you reuse the rejected water?"]}</p>
          )}
          <div className="radio-group space-x-8">
            {["Yes", "No"].map((option) => (
              <label key={option} className="flex items-center text-white text-xl">
